...
public class StatsService extends Service {
    ...
    public enum StatsEndpointsEnum {
        ...
        public StatsEvents.SectionUpdatedAbstract getEndpointUpdateEvent(final SiteModel site,
            ...
            switch (this) {
                case VISITS:
                    return new StatsEvents.VisitorsAndViewsUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (VisitsModel)data);
                case TOP_POSTS:
                    return new StatsEvents.TopPostsUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (TopPostsAndPagesModel)data);
                case REFERRERS:
                    return new StatsEvents.ReferrersUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (ReferrersModel)data);
                case CLICKS:
                    return new StatsEvents.ClicksUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (ClicksModel)data);
                case AUTHORS:
                    return new StatsEvents.AuthorsUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (AuthorsModel)data);
                case GEO_VIEWS:
                    return new StatsEvents.CountriesUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (GeoviewsModel)data);
                case VIDEO_PLAYS:
                    return new StatsEvents.VideoPlaysUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (VideoPlaysModel)data);
                case SEARCH_TERMS:
                    return new StatsEvents.SearchTermsUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (SearchTermsModel)data);
                case COMMENTS:
                    return new StatsEvents.CommentsUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (CommentsModel)data);
                case COMMENT_FOLLOWERS:
                    return new StatsEvents.CommentFollowersUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (CommentFollowersModel)data);
                case TAGS_AND_CATEGORIES:
                    return new StatsEvents.TagsUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (TagsContainerModel)data);
                case PUBLICIZE:
                    return new StatsEvents.PublicizeUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (PublicizeModel)data);
                case FOLLOWERS_WPCOM:
                    return new StatsEvents.FollowersWPCOMUdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (FollowersModel)data);
                case FOLLOWERS_EMAIL:
                    return new StatsEvents.FollowersEmailUdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (FollowersModel)data);
                case INSIGHTS_POPULAR:
                    return new StatsEvents.InsightsPopularUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (InsightsPopularModel)data);
                case INSIGHTS_ALL_TIME:
                    return new StatsEvents.InsightsAllTimeUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (InsightsAllTimeModel)data);
                case INSIGHTS_TODAY:
                    return new StatsEvents.VisitorsAndViewsUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (VisitsModel)data);
                case INSIGHTS_LATEST_POST_SUMMARY:
                    return new StatsEvents.InsightsLatestPostSummaryUpdated(site, timeframe, date,
                            maxResultsRequested, pageRequested, (InsightsLatestPostModel)data);
                case INSIGHTS_LATEST_POST_VIEWS:
                    return new StatsEvents.InsightsLatestPostDetailsUpdated(site, timeframe, date,
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        final SiteModel site = (SiteModel) intent.getExtras().getSerializable(ActivityLauncher.EXTRA_SITE);
        if (site == null) {
            AppLog.e(T.STATS, "StatsService was started with a null site");
            return START_NOT_STICKY;
        }
        ...
        if (intent.getStringExtra(ARG_DATE) == null) {
            AppLog.w(T.STATS, "StatsService is started with a NULL date on this siteId - "
                    + site.getSiteId() + ". Using current date!!!");
        ...
        for (int i=0; i < sectionFromIntent.length; i++){
            ...
            singleThreadNetworkHandler.submit(new Thread() {
                @Override
                public void run() {
                    startTasks(site, period, requestedDate, currentSectionsToUpdate, maxResultsRequested, pageRequested);
                }
            });
        }
    ...
    private String getCachedStats(final SiteModel site, final StatsTimeframe timeframe, final String date,
                                  final StatsEndpointsEnum sectionToUpdate,
                                  final int maxResultsRequested, final int pageRequested) {
        if (!isCacheEnabled()) {
            return null;
        }
        return StatsTable.getStats(this, site, timeframe, date, sectionToUpdate, maxResultsRequested, pageRequested);
    }

    private void startTasks(final SiteModel site, final StatsTimeframe timeframe, final String date,
        ...
        String cachedStats = getCachedStats(site, timeframe, date, sectionToUpdate, maxResultsRequested, pageRequested);
        if (cachedStats != null) {
            BaseStatsModel mResponseObjectModel;
                try {
                    JSONObject response = new JSONObject(cachedStats);
                    mResponseObjectModel = StatsUtils.parseResponse(sectionToUpdate, site, response);

                    EventBus.getDefault().post(
                            sectionToUpdate.getEndpointUpdateEvent(site, timeframe, date,
                                    maxResultsRequested, pageRequested, mResponseObjectModel)
                    );

                    updateWidgetsUI(site, sectionToUpdate, timeframe, date, pageRequested, mResponseObjectModel);
...
*/
        ...
        RestListener vListener = new RestListener(sectionToUpdate, site, timeframe, date, maxResultsRequested, pageRequested);

        final String periodDateMaxPlaceholder =  "?period=%s&date=%s&max=%s";

        String path = String.format("/sites/%s/stats/" + sectionToUpdate.getRestEndpointPath(), site.getSiteId());
        synchronized (mStatsNetworkRequests) {
            switch (sectionToUpdate) {
                ...
                case INSIGHTS_LATEST_POST_SUMMARY:
                    ...
                    path = String.format("/sites/%s/%s", site.getSiteId(), sectionToUpdate.getRestEndpointPath()
    ...
    private void updateWidgetsUI(SiteModel site, final StatsEndpointsEnum endpointName,
        ...
        if (!StatsWidgetProvider.isBlogDisplayedInWidget(site)) {
            AppLog.d(AppLog.T.STATS, "The blog with remoteID " + site.getSiteId() + " is NOT displayed in any " +
                    "widget. Stats Service doesn't call an update of the widget.");
            return;
        }
    ...
    private class RestListener implements RestRequest.Listener, RestRequest.ErrorListener {
        ...
        public RestListener(StatsEndpointsEnum endpointName, SiteModel site, StatsTimeframe timeframe, String date,
                            final int maxResultsRequested, final int pageRequested) {
            mSite = site;
                ...
                public void run() {
                    ...
                    if (response != null) {
                        try {
                            ...
                            mResponseObjectModel = StatsUtils.parseResponse(mEndpointName, mSite, response);
                            if (isCacheEnabled()) {
                                StatsTable.insertStats(StatsService.this, mSite, mTimeframe, mDate, mEndpointName,
                    ...
                    EventBus.getDefault().post(
                            mEndpointName.getEndpointUpdateEvent(mSite, mTimeframe, mDate,
                            mMaxResultsRequested, mPageRequested, mResponseObjectModel)
                    );

                    updateWidgetsUI(mSite, mEndpointName, mTimeframe, mDate, mPageRequested, mResponseObjectModel);
        ...
        public void onErrorResponse(final VolleyError volleyError) {
            singleThreadNetworkHandler.submit(new Thread() {
                ...
                public void run() {
                    ...
                    EventBus.getDefault().post(new StatsEvents.SectionUpdateError(mEndpointName, mSite, mTimeframe, mDate,
